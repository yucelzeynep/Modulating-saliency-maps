...
import saliency_tools as saliency_tools
import fig_tools as fig_tools
...
class Person():
    """
    Person is a collection of data of a single participant
    It involves all image names and gaze data
    """
    def __init__(self):
        self.name = []
        self.age = []  
        ...
        for object_type in constants.OBJECT_TYPES:
            ...
            (target and blank) are displayed multile times, so I cannot use 
            simply their names as dictionary keys
            
            For initializing fixation map, I use screen_height and screen_width.
            Actualy these are the same as image_height and image_width,
            since I display the images in full screen.
            But this is not necessarily true for any experiment.
...
#            pickle.dump(self, f, pickle.HIGHEST_PROTOCOL)
    ...
    def load_age(self, participant):
        """
        This functions load the age and sets the age_range.
        
        Anyone younger than 30 has an age range of young,
        and others have an age_range of old.
    ...
    def load_image_related(self,participant):
        ...
        (target and blank) are displayed multiple times, so I cannot use simply
        ...
        gaze_fname = file_tools.find_file_in_path('*_gaze.txt', gaze_path)[0]
        ...
        Here, t0 is the instant when the image is displayed and tf is the 
        instant when it is removed. 
        """
        timelog_fnames = np.sort(file_tools.find_file_in_path('*_timelog.txt', gaze_path))            
        ...
        for object_type in preferences.OBJECT_TYPES_INTEREST:
            for image_fname, image_fixation in zip(\
                ...
                image_fname != constants.BLANK_IMAGE_FNAME:
                    ...
                    fmap = saliency_tools.Fixpos2Densemap(image_fixation, \
                    ...
                    fig_tools.scale_and_display(myobject.image_orig, 'image_orig')
                    fig_tools.scale_and_display(image_temp, 'image_with_fixations')
                    fig_tools.scale_and_display(fmap, 'fmap')               
                            
                else:
                    # for target and blank image, I donot care about fmaps
                    fmap = []
    ...
    def load_familiarities(self, participant):
        ...
        A familiarity of -1 indicates that no value is entered. In this case:
            1. The image is blank or target.
            2. The particpant did not see that page and did not write any scores 
            (There is one such person. Her name is not released due to privacy concerns)
